...
refresh_lcd() {
    switch (graphics_mode) {
        ...
        case GRAPHICSMODE_DEFAULT: {
            ...
            lcd_set_window(graphics_buffer_shift_x, graphics_buffer_shift_y*2, graphics_buffer_width,
                           graphics_buffer_height*2);
            uint32_t i = graphics_buffer_width * graphics_buffer_height;
            start_pixels();
            for (int y = 0; y < graphics_buffer_height * 2; y++)
                for (int x = 0; x < graphics_buffer_width; x++) {
                    st7789_lcd_put_pixel(pio, sm, palette[graphics_buffer[x + (y / 2) * graphics_buffer_width]]);
                    st7789_lcd_put_pixel(pio, sm, palette[graphics_buffer[x + (y / 2) * graphics_buffer_width]]);
                }
