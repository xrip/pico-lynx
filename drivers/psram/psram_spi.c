...
void psram_memset(uint32_t addr32, uint32_t val, uint32_t size) {
    uint32_t eaddr = addr32 + size;
    for (; addr32 % 4; ++addr32) {
...
void psram_memcpy(char* paddr, uint32_t size) {
    uint32_t addr32 = (uint32_t)paddr;
    uint32_t eaddr = addr32 + size;
    for (; addr32 % 4; ++addr32) {
        *paddr++ = psram_read8(&psram_spi, addr32);
    }
    uint32_t* paddr32 = (uint32_t*)paddr;
    for (; addr32 < eaddr; addr32 += 4) {
        *paddr32++ = psram_read32(&psram_spi, addr32);
    }
    addr32 -= 3;
    paddr = (char*)paddr32;
    for (; addr32 < eaddr; ++addr32) {
