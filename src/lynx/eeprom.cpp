...
bool CEEPROM::ContextSave(LSS_FILE *fp)
{
   ...
   if(!lss_write(&busy_count, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&state, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&readdata,sizeof(UWORD),1,fp)) return 0;

   if(!lss_write(&data, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&addr,sizeof(UWORD),1,fp)) return 0;
   if(!lss_write(&sendbits, sizeof(ULONG2), 1, fp)) return 0;
...
bool CEEPROM::ContextLoad(LSS_FILE *fp)
{
   ...
   if(!lss_read(&busy_count, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&state, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&readdata,sizeof(UWORD),1,fp)) return 0;

   if(!lss_read(&data, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&addr,sizeof(UWORD),1,fp)) return 0;
   if(!lss_read(&sendbits, sizeof(ULONG2), 1, fp)) return 0;
