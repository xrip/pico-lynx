...
CCart::CCart(UBYTE *gamedata, ULONG2 gamesize)
{
...
bool CCart::ContextSave(LSS_FILE *fp)
{
   ...
   if(!lss_write(&mCounter, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mShifter, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAddrData, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mStrobe, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mShiftCount0, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mCountMask0, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mShiftCount1, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mCountMask1, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mBank,sizeof(EMMODE),1,fp)) return 0;
   if(!lss_write(&mDummy, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mWriteEnableBank1, sizeof(ULONG2), 1, fp)) return 0;
...
bool CCart::ContextLoad(LSS_FILE *fp)
{
   ...
   if(!lss_read(&mCounter, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mShifter, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAddrData, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mStrobe, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mShiftCount0, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mCountMask0, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mShiftCount1, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mCountMask1, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mBank,sizeof(EMMODE),1,fp)) return 0;
   if(!lss_read(&mDummy, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mWriteEnableBank1, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mCartRAM, sizeof(ULONG2), 1, fp)) return 0;
   if(mCartRAM) {
...
inline void CCart::Poke(ULONG2 addr, UBYTE data)
{
...
inline UBYTE CCart::Peek(ULONG2 addr)
{
   if(mBank==bank0) {
...
void CCart::Poke1(UBYTE data)
{
   if(mWriteEnableBank1) {
      ULONG2 address= (mShifter << mShiftCount1) + (mCounter & mCountMask1);
...
{
   ULONG2 address= (mShifter << mShiftCount0) + (mCounter & mCountMask0);
