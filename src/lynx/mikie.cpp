...
static inline ULONG2 GetLfsrNext(ULONG2 current)
{
   ...
   static ULONG2 switches,lfsr,next,swloop,result;
   static const ULONG2 switchbits[9]={ 7, 0, 1, 2, 3, 4, 5, 10, 11};
...
CMikie::CMikie(CSystem& parent, ULONG2 displayformat, ULONG2 samplerate)
:mSystem(parent)
{
   TRACE_MIKIE0("CMikie()");
   ...
   mDisplayPitch=HANDY_SCREEN_WIDTH * 1;
...
bool CMikie::ContextSave(LSS_FILE *fp)
{
   ...
   if(!lss_write(&mDisplayAddress, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAudioInputComparator, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTimerStatusFlags, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTimerInterruptMask, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(mPalette,sizeof(TPALETTE),16,fp)) return 0;

   if(!lss_write(&mIODAT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mIODAT_REST_SIGNAL, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mIODIR, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mDISPCTL_DMAEnable, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mDISPCTL_Flip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mDISPCTL_FourColour, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mDISPCTL_Colour, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_0_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_0_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_1_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_1_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_2_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_2_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_3_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_3_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_4_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_4_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_5_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_5_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_6_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_6_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mTIM_7_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mTIM_7_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mAUDIO_0_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_OUTPUT[0],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_0_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_0_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mAUDIO_1_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_OUTPUT[1],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_1_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_1_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mAUDIO_2_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_OUTPUT[2],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_2_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_2_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mAUDIO_3_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_OUTPUT[3],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_write(&mAUDIO_3_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mAUDIO_3_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSTEREO, sizeof(ULONG2), 1, fp)) return 0;

   //
   // Serial related variables
   //
   if(!lss_write(&mUART_RX_IRQ_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_TX_IRQ_ENABLE, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mUART_TX_COUNTDOWN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_RX_COUNTDOWN, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mUART_SENDBREAK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_TX_DATA, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_RX_DATA, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_RX_READY, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mUART_PARITY_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mUART_PARITY_EVEN, sizeof(ULONG2), 1, fp)) return 0;
...
bool CMikie::ContextLoad(LSS_FILE *fp)
{
   ...
   if(!lss_read(&mDisplayAddress, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAudioInputComparator, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTimerStatusFlags, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTimerInterruptMask, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(mPalette,sizeof(TPALETTE),16,fp)) return 0;

   if(!lss_read(&mIODAT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mIODAT_REST_SIGNAL, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mIODIR, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mDISPCTL_DMAEnable, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mDISPCTL_Flip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mDISPCTL_FourColour, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mDISPCTL_Colour, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_0_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_0_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_1_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_1_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_2_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_2_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_3_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_3_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_4_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_4_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_5_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_5_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_6_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_6_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mTIM_7_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mTIM_7_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mAUDIO_0_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_OUTPUT[0],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_0_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_0_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mAUDIO_1_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_OUTPUT[1],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_1_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_1_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mAUDIO_2_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_OUTPUT[2],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_2_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_2_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mAUDIO_3_BKUP, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_ENABLE_RELOAD, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_ENABLE_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_LINKING, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_CURRENT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_TIMER_DONE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_LAST_CLOCK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_BORROW_IN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_BORROW_OUT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_LAST_LINK_CARRY, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_LAST_COUNT, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_VOLUME,sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_OUTPUT[3],sizeof(SBYTE),1,fp)) return 0;
   if(!lss_read(&mAUDIO_3_INTEGRATE_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mAUDIO_3_WAVESHAPER, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSTEREO, sizeof(ULONG2), 1, fp)) return 0;

   //
   // Serial related variables
   //
   if(!lss_read(&mUART_RX_IRQ_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_TX_IRQ_ENABLE, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mUART_TX_COUNTDOWN, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_RX_COUNTDOWN, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mUART_SENDBREAK, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_TX_DATA, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_RX_DATA, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_RX_READY, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mUART_PARITY_ENABLE, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mUART_PARITY_EVEN, sizeof(ULONG2), 1, fp)) return 0;
   return 1;
}
...
void CMikie::ComLynxTxCallback(void (*function)(int data, ULONG2 objref), ULONG2 objref)
{
...
inline ULONG2 CMikie::DisplayRenderLine(void)
{
   ...
   if(mLynxLineDMACounter) {
      ...
      bitmap_tmp=(UBYTE*)mpDisplayCurrent;
		...
		switch(mDisplayRotate)
		{
         case MIKIE_ROTATE_L:
            for(loop=0;loop<HANDY_SCREEN_WIDTH/2;loop++)
            {
               ...
               {
                  mLynxAddr++;
                  if (bitmap_tmp >= (UBYTE*)(gPrimaryFrameBuffer))
                  *(bitmap_tmp)=mColourMap[mPalette[source>>4].Index];
                  bitmap_tmp-=HANDY_SCREEN_WIDTH;
                  if (bitmap_tmp >= (UBYTE*)(gPrimaryFrameBuffer))
                  *(bitmap_tmp)=mColourMap[mPalette[source&0x0f].Index];
            ...
            for(loop=0;loop<HANDY_SCREEN_WIDTH/2;loop++)
            {
               ...
               {
                  mLynxAddr--;
                  *(bitmap_tmp++)=mColourMap[mPalette[source&0x0f].Index] & 0xff;
                  *(bitmap_tmp++)=mColourMap[mPalette[source>>4].Index] & 0xff;
               }
               else
               {
                  mLynxAddr++;
                  // *(bitmap_tmp++)=mapping[mPalette[source>>4].Index];
                  *(bitmap_tmp++)=mColourMap[mPalette[source>>4].Index] & 0xff;
                  *(bitmap_tmp++)=mColourMap[mPalette[source&0x0f].Index] & 0xff;
               }
            }
...
void CMikie::Poke(ULONG2 addr, UBYTE data)
{
   switch(addr&0xff) {
      case (TIM0BKUP&0xff):
         mTIM_0_BKUP=data;
      ...
      case (AUD0SHFTFB&0xff):
         ...
         mAUDIO_0_WAVESHAPER|=(ULONG2)data << 13;
      ...
      case (AUD1SHFTFB&0xff):
         ...
         mAUDIO_1_WAVESHAPER|=(ULONG2)data << 13;
      ...
      case (AUD2SHFTFB&0xff):
         ...
         mAUDIO_2_WAVESHAPER|=(ULONG2)data << 13;
      ...
      case (AUD3SHFTFB&0xff):
         ...
         mAUDIO_3_WAVESHAPER|=(ULONG2)data << 13;
