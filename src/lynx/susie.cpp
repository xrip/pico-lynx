...
bool CSusie::ContextSave(LSS_FILE *fp)
{
   ...
   if(!lss_write(&mSPRCTL0_Type, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL0_Vflip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL0_Hflip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL0_PixelBits, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSPRCTL1_StartLeft, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_StartUp, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_SkipSprite, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_ReloadPalette, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_ReloadDepth, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_Sizing, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCTL1_Literal, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSPRCOLL_Number, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRCOLL_Collide, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSPRSYS_StopOnCurrent, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_LeftHand, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_VStretch, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_NoCollide, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_Accumulate, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_SignedMath, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_Status, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_UnsafeAccess, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_LastCarry, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_Mathbit, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mSPRSYS_MathInProgress, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSUZYBUSEN, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mSPRINIT,sizeof(TSPRINIT),1,fp)) return 0;

   if(!lss_write(&mSPRGO, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mEVERON, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(mPenIndex,sizeof(UBYTE),16,fp)) return 0;

   if(!lss_write(&mLineType, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLineShiftRegCount, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLineShiftReg, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLineRepeatCount, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLinePixel, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLinePacketBitsLeft, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mCollision, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_write(&mLineBaseAddress, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_write(&mLineCollisionAddress, sizeof(ULONG2), 1, fp)) return 0;
...
bool CSusie::ContextLoad(LSS_FILE *fp)
{
   ...
   if(!lss_read(&mSPRCTL0_Type, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL0_Vflip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL0_Hflip, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL0_PixelBits, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSPRCTL1_StartLeft, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_StartUp, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_SkipSprite, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_ReloadPalette, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_ReloadDepth, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_Sizing, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCTL1_Literal, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSPRCOLL_Number, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRCOLL_Collide, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSPRSYS_StopOnCurrent, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_LeftHand, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_VStretch, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_NoCollide, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_Accumulate, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_SignedMath, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_Status, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_UnsafeAccess, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_LastCarry, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_Mathbit, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mSPRSYS_MathInProgress, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSUZYBUSEN, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mSPRINIT,sizeof(TSPRINIT),1,fp)) return 0;

   if(!lss_read(&mSPRGO, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mEVERON, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(mPenIndex,sizeof(UBYTE),16,fp)) return 0;

   if(!lss_read(&mLineType, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLineShiftRegCount, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLineShiftReg, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLineRepeatCount, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLinePixel, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLinePacketBitsLeft, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mCollision, sizeof(ULONG2), 1, fp)) return 0;

   if(!lss_read(&mLineBaseAddress, sizeof(ULONG2), 1, fp)) return 0;
   if(!lss_read(&mLineCollisionAddress, sizeof(ULONG2), 1, fp)) return 0;
...
ULONG2 CSusie::PaintSprites(void)
{
   int	sprcount=0;
   int data=0;
   int everonscreen=0;
               ...
               for(;;) {
                  ...
                  for(vloop=0;vloop<pixel_height;vloop++) {
                     ...
                     if(voff>=0 && voff<HANDY_SCREEN_HEIGHT) {
                        ...
                        ULONG2 pixel = mLinePixel; // Much faster
                        switch(mSPRCTL0_Type)
                        {
                              ...
                              case sprite_boundary:
                                 ...
                                 { \
                                    if(!mSPRCOLL_Collide && !mSPRSYS_NoCollide) \
                                    { \
                                       ULONG2 collision=ReadCollision(hoff); \
...
void CSusie::Poke(ULONG2 addr, UBYTE data)
{
   switch(addr&0xff) {
      case (TMPADRL&0xff):
      ...
      case (MATHA&0xff):
         ...
         mMATHEFGH.Long= (ULONG2)mMATHABCD.Words.AB * (ULONG2)mMATHABCD.Words.CD;
         ...
         if(mSPRSYS_Accumulate) {
            ...
            ULONG2 tmp= mMATHJKLM.Long + mMATHEFGH.Long;
