...
//
ULONG2   gSystemCycleCount=0;
ULONG2   gNextTimerEvent=0;
ULONG2   gCPUWakeupTime=0;
ULONG2   gIRQEntryCycle=0;
ULONG2   gCPUBootAddress=0;
ULONG2   gBreakpointHit=FALSE;
ULONG2   gSingleStepMode=FALSE;
ULONG2   gSingleStepModeSprites=FALSE;
ULONG2   gSystemIRQ=FALSE;
ULONG2   gSystemNMI=FALSE;
ULONG2   gSystemCPUSleep=FALSE;
ULONG2   gSystemCPUSleep_Saved=FALSE;
ULONG2   gSystemHalt=FALSE;
ULONG2   gThrottleMaxPercentage=100;
ULONG2   gThrottleLastTimerCount=0;
ULONG2   gThrottleNextCycleCheckpoint=0;
ULONG2   gEndOfFrame=0;
ULONG2   gTimerCount=0;
...
bool CSystem::ContextSave(LSS_FILE *fp)
{
   ...
   // Save ROM CRC
   ULONG2 checksum=mCart->CRC32();
   if(!lss_write(&checksum, sizeof(ULONG2), 1, fp)) status=0;

   if(!lss_printf(fp, "CSystem::ContextSave")) status=0;

   if(!lss_write(&mCycleCountBreakpoint, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemCycleCount, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gNextTimerEvent, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gCPUWakeupTime, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gCPUBootAddress, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gIRQEntryCycle, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gBreakpointHit, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSingleStepMode, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemIRQ, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemNMI, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemCPUSleep, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemCPUSleep_Saved, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gSystemHalt, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gThrottleMaxPercentage, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gThrottleLastTimerCount, sizeof(ULONG2), 1, fp)) status=0;
   if(!lss_write(&gThrottleNextCycleCheckpoint, sizeof(ULONG2), 1, fp)) status=0;

   if(!lss_write(&gTimerCount, sizeof(ULONG2), 1, fp)) status=0;

   if(!lss_write(&gAudioLastUpdateCycle, sizeof(ULONG2), 1, fp)) status=0;
...
bool CSystem::ContextLoad(LSS_FILE *fp)
{
   ...
   if(strcmp(teststr,LSS_VERSION)==0) {
      ...
      if(!lss_read(&mCycleCountBreakpoint, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemCycleCount, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gNextTimerEvent, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gCPUWakeupTime, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gCPUBootAddress, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gIRQEntryCycle, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gBreakpointHit, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSingleStepMode, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemIRQ, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemNMI, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemCPUSleep, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemCPUSleep_Saved, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gSystemHalt, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gThrottleMaxPercentage, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gThrottleLastTimerCount, sizeof(ULONG2), 1, fp)) status=0;
      if(!lss_read(&gThrottleNextCycleCheckpoint, sizeof(ULONG2), 1, fp)) status=0;

      if(!lss_read(&gTimerCount, sizeof(ULONG2), 1, fp)) status=0;

      if(!lss_read(&gAudioLastUpdateCycle, sizeof(ULONG2), 1, fp)) status=0;
