...
class C65C02
{
   ...
   public:
      ...
      inline bool ContextSave(LSS_FILE *fp)
      {
         ...
         if(!lss_write(&mA, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mX, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mY, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mSP, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mPS, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mPC, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_write(&mIRQActive, sizeof(ULONG2), 1, fp)) return 0;
         return 1;
      }
      ...
      inline bool ContextLoad(LSS_FILE *fp)
      {
         ...
         if(!lss_read(&mA, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mX, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mY, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mSP, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mPS, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mPC, sizeof(ULONG2), 1, fp)) return 0;
         if(!lss_read(&mIRQActive, sizeof(ULONG2), 1, fp)) return 0;
         PS(mPS);
         return 1;
      }
